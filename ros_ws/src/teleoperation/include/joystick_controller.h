...
constexpr const char* PARAMETER_JOYSTICK_CRAB_BTN = "joystick_crab_button";
...
class JoystickController
{
     ...
     */
    float m_acceleration_scaling_factor = 1.f;
     ...
     */
    float m_deceleration_scaling_factor = -1.f;
     ...
     */
    float m_steering_scaling_factor = -1.f;
	...
	int crabModeButton;
        int enableManualButton; // AKA DMS (dead-man's switch)
        int enableAutonomousButton;
        std::string name;
    };

    const struct JoystickMapping joystick_mapping_nin = { 0, 1, 5, 4, 9, "nint" };
    const struct JoystickMapping joystick_mapping_ps3 = { 0, 13, 12, 14, 13, "ps3" };
    const struct JoystickMapping joystick_mapping_xbox360 = { 0, 4, 3, 0, 1, "xbox360" };
    const struct JoystickMapping joystick_mapping_xboxone = { 0, 5, 3, 0, 1, "xbone" };
     ...
     * @param steering_angle The steering angle provided by the gamepad input
     @ @param Whether to steer in crab mode
     */
    void publishDriveParameters(double velocity, double steering_angle, bool crabMode);
